...
export const db = {
   ...
   *
   * `created_at DESC, id DESC` rather than `updated_at`: these rows are never
   * updated, and `id` breaks ties deterministically so two snapshots written in
   * the same millisecond don't swap places between reads.
   */
  ...
  /**
   * Append one snapshot. The only way a version row is ever written — there is
   * deliberately no update or reorder, because a snapshot is a record of what
   * the document was, and a history you can rewrite answers no question.
   */
  ...
  async getDirty(): Promise<SyncPayload> {
    const database = await getDb();
    const [folders, notes, copa_items, issues, resume_versions] = await Promise.all([
    ...
    return { folders, notes, copa_items, issues, resume_versions };
  },
  ...
  async applyServerRows(payload: SyncPayload): Promise<number> {
    ...
    await database.withTransactionAsync(async () => {
      ...
      for (const v of payload.resume_versions) {
        // Same last-writer-wins guard as every other table, though it should
        // never fire in anger here: these rows are immutable, so the only row
        // that ever conflicts with an incoming one is a byte-identical copy of
        // itself (a re-push after a dropped response). The guard being `>=`
        // rather than `>` is what makes that resend harmless.
        const r = await database.runAsync(
  ...
  async markAllDirty(): Promise<void> {
    ...
    await database.execAsync(
      'UPDATE folders SET dirty = 1; UPDATE notes SET dirty = 1; UPDATE copa_items SET dirty = 1; UPDATE issues SET dirty = 1; UPDATE resume_versions SET dirty = 1;',
    );
  },
  ...
  async clearAllData(): Promise<void> {
    ...
    await database.execAsync(
      'DELETE FROM folders; DELETE FROM notes; DELETE FROM copa_items; DELETE FROM issues; DELETE FROM resume_versions;',
    );
  },
